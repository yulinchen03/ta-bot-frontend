<template>
  <div class="w-screen h-screen bg-gray-200 p-10 flex justify-center align-middle flex-col">
    ...
    <transition>
      <div
        v-if="feedbackFormVisible"
        class="box-border gap-6 h-min-56 flex flex-col justify-center align-middle z-50 w-1/2 h-fit bg-white rounded-xl shadow-xl fixed bottom-2 inset-0 m-auto"
      >
        <div class="flex flex-row bg-ut-green rounded-t-xl px-2">
          ...
          <el-icon
            ...
            class="hover:cursor-pointer self-end my-auto"
          >
            <svg
              class="text-white"
        ...
        <div class="p-3 w-full flex flex-col gap-6">
          <input
            ...
            v-model="feedback"
            class="w-full rounded-lg p-1 border"
          />
          <button
            class="self-end rounded-lg w-fit bg-ut-green text-white hover:cursor-pointer px-2 py-1"
            @click="sendFeedback"
          >
            Send
          </button>
    ...
    <div class="flex flex-row w-full justify-between align-middle">
      <div
        class="my-auto h-fit flex hover:cursor-pointer"
        @click="this.$router.push({ path: 'course', query: { id: courseId } })"
      >
        <el-icon :size="32" class="m-auto w-8">
          <ArrowLeft />
        </el-icon>
        <div class="text-3xl">Course</div>
      </div>
      <div
        v-if="exercises.length !== 0"
        @click="showExercises = !showExercises"
        class="w-1/4 border flex flex-row justify-between p-4 bg-white rounded-full relative"
      >
        <div class="text-2xl">
          {{
            this.exercises.find((exercise) => exercise.id === parseInt(this.exerciseId)).identifier
          }}
        </div>
        <div
          @click="showExercises = !showExercises"
          v-if="showExercises"
          class="shadow-lg bg-white z-30 text-2xl rounded-lg p-2 w-full h-max-1/4 scroll-auto absolute left-0 top-full"
        >
          <div
            ...
            @click="
              this.$router.push({
                path: '/bot',
                query: { courseId, assignmentId, exerciseId: exercise.id }
              })
            "
            :class="{
              'hover:bg-ut-blue hover:text-white hover:cursor-pointer p-2': true,
              'border-b-2': index < exercises.length - 1
            }"
          >
            {{ exercise.identifier }}
          </div>
        ...
        <el-icon class="transition transform ease-in-out delay-150 hover:cursor-pointer" :size="32">
          <svg
            :class="{ 'transform rotate-180': showExercises }"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 1024 1024"
            data-v-ea893728=""
          >
    ...
    <div class="w-full h-full flex flex-row justify-center align-middle">
      <div class="h-4/5 flex flex-row m-auto w-full">
        <div
          class="rounded-3xl custom-scrollbar overflow-hidden h-full flex w-2/3 outline outline-gray-400 outline-2 bg-white m-auto relative flex-col"
        >
        ...
        <div class="m-auto w-1/3 p-3 box-border h-full overflow-y-auto">
          <div class="md:flex-col lg:flex-row flex justify-between w-full">
            <div
              @click="previous"
              class="gap-4 md:text-lg lg:text-2xl flex justify-center flex-row w-full hover:bg-ut-gold outline outline-ut-gold outline-2 p-3 m-3 box-border rounded-xl bg-ut-white hover:text-white text-ut-gold hover:cursor-pointer text-center"
            >
              <svg
                class="w-10"
                xmlns="http://www.w3.org/2000/svg"
            ...
            <div
              ...
              class="gap-4 md:text-lg lg:text-2xl flex justify-center flex-row w-full hover:bg-ut-green outline outline-ut-green outline-2 p-3 m-3 box-border rounded-xl bg-ut-white hover:text-white text-ut-green hover:cursor-pointer text-center"
            >
              <svg
                class="w-10"
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div
            ...
            :key="option.next_hint"
            class="text-lg w-full bg-white outline outline-ut-blue outline-2 p-3 my-3 box-border rounded-xl hover:bg-ut-blue text-ut-blue hover:text-white hover:cursor-pointer"
          >
            {{ option.option }}
          </div>
        </div>
