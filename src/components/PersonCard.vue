<template>
  <div
    ...
    <div class="flex w-full items-center">
      <img :src="'https://xsgames.co/randomusers/avatar.php?g=pixel&' + user.id"
           class="w-12 h-12 aspect-square rounded-full mr-6"/>
      <div class="flex items-center gap-4">
        <div class="grid grid-cols-1 items-center">
          <div v-if="!showEdit" class="text-lg w-fit h-fit"><b>{{ user.name }} {{ user.surname }} <span
              class="text-lg text-ut-gold">({{ user.role }})</span></b></div>
        ...
        <el-form :inline="true" style="width: 35vw" v-if="showEdit">
          <el-form-item label="Name" class="editUserFormItem" label-width="auto">
            <el-input @keydown="preventEnter" v-model="user.name" v-if="showEdit" placeholder="name"
                      style="width: 10vw"/>
          </el-form-item>
          <el-form-item label="Surname" class="editUserFormItem" label-width="auto">
            <el-input @keydown="preventEnter" v-model="user.surname" v-if="showEdit" placeholder="surname"
                      style="width: 10vw"/>
          </el-form-item>
          <el-form-item label="Email address" class="editUserFormItem" label-width="auto">
            <el-input @keydown="preventEnter" v-model="user.email" v-if="showEdit" placeholder="email"
                      style="width: 23vw"/>
          </el-form-item>
        </el-form>
        <el-button class="custom-button" @keypress.enter.native.prevent
                   @click="editUserAdmin(user.id, { name: user.name, surname: user.surname, email: user.email})"
                   v-if="showEdit">Save
        </el-button>
      </div>
    </div>
